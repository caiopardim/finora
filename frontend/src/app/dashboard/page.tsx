...
export default function DashboardPage() {
  ...
  async function load() {
    ...
    // Forecast = sum of budgets if set (planned spending); fallback to pace extrapolation
      ...
      {txs.map((tx:any, i:number) => (
        <div key={tx.id} style={{ display:'flex', alignItems:'center', gap:14, padding:'11px 0', borderBottom:i<txs.length-1?`1px solid ${c.borderLight}`:'none' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:4, flexShrink:0 }}>
            ...
            <button onClick={() => setConfirmId(tx.id)} style={{ background:'none', border:'none', cursor:'pointer', color:'#ef4444', padding:'0 2px', fontSize:14 }}>✕</button>
          </div>
        </div>
      ))}
    </div>
